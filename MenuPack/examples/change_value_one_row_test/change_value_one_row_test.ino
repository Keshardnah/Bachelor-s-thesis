...
void setup() {
  ...
  change_value = menu_pack->makeMenu(2);
  menu_element *element = new menu_element;
  element = menu_pack->makeMenuElement("X (int):", ME_INT);
  ...
  element = menu_pack->makeMenuElement("Y (float):", ME_FLOAT);
  ...
  selected_values = menu_pack->makeMenu(2);
...
void firstPrintSelectedValues(menu *selected_values, menu *change_value) {
  ...
  menu_element *element = new menu_element;
  String *value = new String(*(int*)(menu_pack->getValue(menu_pack->getMenuElement(change_value, 0))));
  String name = "Current X:";
  name.concat(*value);
  element = menu_pack->makeMenuElement(name, ME_MENU);
  menu_pack->addElementToMenu(element, selected_values);
  value = new String(*(float*)(menu_pack->getValue(menu_pack->getMenuElement(change_value, 1))));
  name = "Current Y:";
  name.concat(*value);
  element = menu_pack->makeMenuElement(name, ME_MENU);
  menu_pack->addElementToMenu(element, selected_values);
